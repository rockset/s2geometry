...
#include <type_traits>
#include <vector>
#include "s2/third_party/xbsl/base/internal/unaligned_access.h"
...
template <class T>
void EncodeUintVector(xbsl::Span<const T> v, Encoder* encoder);
...
inline T GetUintWithLength(const char* ptr, int length) {
  ...
  if (length & sizeof(T)) {
    if (sizeof(T) == 8) return XBSL_INTERNAL_UNALIGNED_LOAD64(ptr);
    if (sizeof(T) == 4) return XBSL_INTERNAL_UNALIGNED_LOAD32(ptr);
    if (sizeof(T) == 2) return XBSL_INTERNAL_UNALIGNED_LOAD16(ptr);
  ...
  if (sizeof(T) > 4 && (length & 4)) {
    x = XBSL_INTERNAL_UNALIGNED_LOAD32(ptr -= sizeof(uint32));
  }
  if (sizeof(T) > 2 && (length & 2)) {
    x = (x << 16) + XBSL_INTERNAL_UNALIGNED_LOAD16(ptr -= sizeof(uint16));
  }
