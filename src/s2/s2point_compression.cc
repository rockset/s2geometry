...
#include "s2/third_party/xbsl/container/fixed_array.h"
...
struct FaceRun {
  ...
  void Encode(Encoder* encoder) const {
    ...
    encoder->put_varint64(
        S2CellId::kNumFaces * xbsl::implicit_cast<int64>(count) + face);
...
void S2EncodePointsCompressed(Span<const S2XYZFaceSiTi> points,
                              int level,
                              Encoder* encoder) {
  xbsl::FixedArray<pair<int, int>> vertices_pi_qi(points.size());
