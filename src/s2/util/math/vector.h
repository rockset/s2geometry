...
class BasicVector {
 ...
 protected:
  ...
  using IdxSeqN = typename xbsl::make_index_sequence<N>;
  ...
  static R GenerateEach(xbsl::index_sequence<Is...>, F f, As*... as) {
 ...
 private:
  ...
  static T Dot(T sum, const T* a, const T* b, xbsl::index_sequence<Is...>) {
    Ignore({(sum += a[Is] * b[Is], true)...});
    return sum;
  }

  template <std::size_t... Is>
  static void PlusEq(T* a, const T* b, xbsl::index_sequence<Is...>) {
    Ignore({(a[Is] += b[Is], true)...});
  }

  template <std::size_t... Is>
  static void MinusEq(T* a, const T* b, xbsl::index_sequence<Is...>) {
    Ignore({(a[Is] -= b[Is], true)...});
  }

  template <std::size_t... Is>
  static void MulEq(T* a, T b, xbsl::index_sequence<Is...>) {
    Ignore({(a[Is] *= b, true)...});
  }

  template <std::size_t... Is>
  static void DivEq(T* a, T b, xbsl::index_sequence<Is...>) {
    Ignore({(a[Is] /= b, true)...});
  }
};
