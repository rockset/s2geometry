...
#include "s2/third_party/xbsl/container/internal/compressed_tuple.h"
#include "s2/third_party/xbsl/container/internal/container_memory.h"
...
#include "s2/third_party/xbsl/utility/utility.h"
...
struct is_comparator_transparent<Compare,
                                 xbsl::void_t<typename Compare::is_transparent>>
    : std::true_type {};
...
template <>
struct key_compare_to_adapter<std::less<xbsl::string_view>> {
  struct type : public btree_key_compare_to_tag {
    type() = default;
    explicit type(const std::less<xbsl::string_view> &) {}
    int operator()(const xbsl::string_view a, const xbsl::string_view b) const {
      return a.compare(b);
    }
  };
};
...
template <>
struct key_compare_to_adapter<std::greater<xbsl::string_view>> {
  struct type : public btree_key_compare_to_tag {
    type() = default;
    explicit type(const std::greater<xbsl::string_view> &) {}
    int operator()(const xbsl::string_view a, const xbsl::string_view b) const {
      return b.compare(a);
    }
  };
...
struct has_linear_node_search_preference<
    T, xbsl::void_t<typename T::goog_btree_prefer_linear_node_search>>
    : std::true_type {};
template <typename T>
...
struct common_params {
  ...
  using key_compare =
      xbsl::conditional_t<btree_is_key_compare_to<Compare>::value, Compare,
  ...
  using node_count_type =
      xbsl::conditional_t<(kNodeValueSpace / ValueSize >
...
struct map_params : common_params<Key, Compare, Alloc, TargetNodeSize,
  ...
  using slot_type = xbsl::container_internal::slot_type<Key, mapped_type>;
  ...
  struct value_compare : private key_compare {
    ...
    xbsl::conditional_t<btree_is_key_compare_to<key_compare>::value, int, bool>
...
struct set_params
  ...
  // xbsl::container_internal::slot_type interface.
  struct slot_type {
    value_type value;

    template <class... Args>
    static void construct(Alloc *alloc, slot_type *slot, Args &&... args) {
      xbsl::allocator_traits<Alloc>::construct(*alloc, &slot->value,
    ...
    static void destroy(Alloc *alloc, slot_type *slot) {
      xbsl::allocator_traits<Alloc>::destroy(*alloc, &slot->value);
    }
...
class btree_node {
 ...
 public:
  ...
  using use_linear_search = xbsl::conditional_t<
 ...
 public:
  ...
  void clear_child(int i) {
    xbsl::container_internal::SanitizerPoisonObject(&mutable_child(i));
  }
  void set_child(int i, btree_node *c) {
    xbsl::container_internal::SanitizerUnpoisonObject(&mutable_child(i));
    mutable_child(i) = c;
    c->set_position(i);
  }
  ...
  static btree_node *init_leaf(btree_node *n, btree_node *parent,
    ...
    xbsl::container_internal::SanitizerPoisonMemoryRegion(
 ...
 private:
  ...
  void value_init(const size_type i, allocator_type *alloc, Args &&... args) {
    xbsl::container_internal::SanitizerUnpoisonObject(slot(i));
    slot_type::construct(alloc, slot(i), std::forward<Args>(args)...);
  }
  void value_destroy(const size_type i, allocator_type *alloc) {
    slot_type::destroy(alloc, slot(i));
    xbsl::container_internal::SanitizerPoisonObject(slot(i));
  }
  ...
  void uninitialized_move_n(const size_type n, const size_type i,
                            ...
                            allocator_type *alloc) {
    xbsl::container_internal::SanitizerUnpoisonMemoryRegion(
...
template <typename T>
using is_trivially_copyable = xbsl::conjunction<
    xbsl::is_trivially_copy_constructible<T>,
    xbsl::disjunction<xbsl::is_trivially_copy_assignable<T>,
                      xbsl::negation<std::is_copy_assignable<T>>>,
    xbsl::is_trivially_destructible<T>>;
...
class btree {
  ...
  template <typename K>
  using const_lookup_key_reference = xbsl::conditional_t<
 ...
 public:
  ...
  btree(btree &&x) noexcept
      : root_(std::move(x.root_)),
        rightmost_(xbsl::exchange(x.rightmost_, nullptr)),
        size_(xbsl::exchange(x.size_, 0)) {
 ...
 private:
  ...
  node_type *allocate(const size_type size) {
    return reinterpret_cast<node_type *>(
        xbsl::container_internal::Allocate<node_type::Alignment()>(
  ...
  void deallocate(const size_type size, node_type *node) {
    xbsl::container_internal::Deallocate<node_type::Alignment()>(
 ...
 private:
  ...
  xbsl::container_internal::CompressedTuple<key_compare, allocator_type,
                                            node_type *>
      root_;
  ...
  static_assert(std::is_same<xbsl::result_of_t<key_compare(key_type, key_type)>,
                             xbsl::conditional_t<is_key_compare_to::value, int,
                                                 bool>>::value,
