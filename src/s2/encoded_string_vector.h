...
class StringVectorEncoder {
 public:
  ...
  // REQUIRES: "encoder" uses the default constructor, so that its buffer
  //           can be enlarged as necessary by calling Ensure(int).
  static void Encode(xbsl::Span<const string> v, Encoder* encoder);
...
class EncodedStringVector {
 public:
  ...
  xbsl::string_view operator[](int i) const;
  ...
  // no longer needed.
  std::vector<xbsl::string_view> Decode() const;
...
inline xbsl::string_view EncodedStringVector::operator[](int i) const {
  uint64 start = (i == 0) ? 0 : offsets_[i - 1];
  uint64 limit = offsets_[i];
  return xbsl::string_view(data_ + start, limit - start);
}
