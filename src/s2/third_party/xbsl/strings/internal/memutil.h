...
// (they use xbsl::ascii_tolower instead of tolower).
//
...
#ifndef S2_THIRD_PARTY_XBSL_STRINGS_INTERNAL_MEMUTIL_H_
#define S2_THIRD_PARTY_XBSL_STRINGS_INTERNAL_MEMUTIL_H_

#include <cstddef>
#include <cstring>

#include "s2/third_party/xbsl/base/port.h"  // disable some warnings on Windows
#include "s2/third_party/xbsl/strings/ascii.h"  // for xbsl::ascii_tolower
...
const char* int_memmatch(const char* haystack, size_t haylen,
  ...
  for (; haystack < hayend; ++haystack) {
    char hay = case_sensitive
                   ? *haystack
                   : xbsl::ascii_tolower(static_cast<unsigned char>(*haystack));
    char nee = case_sensitive
                   ? *needle
                   : xbsl::ascii_tolower(static_cast<unsigned char>(*needle));
    if (hay == nee) {
      if (++needle == needleend) {
...
#endif  // S2_THIRD_PARTY_XBSL_STRINGS_INTERNAL_MEMUTIL_H_
