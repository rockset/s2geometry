...
//
// An `xbsl::InlinedVector<T, N>` specifies the default capacity `N` as one of
// its template parameters. Instances where `size() <= N` hold contained
// elements in inline space. Typically `N` is very small so that sequences that
// are expected to be short do not require allocations.
//
// An `xbsl::InlinedVector` does not usually require a specific allocator. If
// the inlined vector grows beyond its initial constraints, it will need to
// allocate (as any normal `std::vector` would). This is usually performed with
// the default allocator (defined as `std::allocator<T>`). Optionally, a custom
// allocator type may be specified as `A` in `xbsl::InlinedVector<T, N, A>`.

#ifndef S2_THIRD_PARTY_XBSL_CONTAINER_INLINED_VECTOR_H_
#define S2_THIRD_PARTY_XBSL_CONTAINER_INLINED_VECTOR_H_
...
// -----------------------------------------------------------------------------
//
// An `xbsl::InlinedVector` is designed to be a drop-in replacement for
// `std::vector` for use cases where the vector's size is sufficiently small
// that it can be inlined. If the inlined vector does grow beyond its estimated
// capacity, it will trigger an initial allocation on the heap, and will behave
// as a `std:vector`. The API of the `xbsl::InlinedVector` within this file is
...
class InlinedVector {
  ...
  using DisableIfIntegral =
      xbsl::enable_if_t<!std::is_integral<Iterator>::value>;

  template <typename Iterator>
  using EnableIfInputIterator = xbsl::enable_if_t<std::is_convertible<
 ...
 public:
  ...
  InlinedVector(InlinedVector&& v) noexcept(
      xbsl::allocator_is_nothrow<allocator_type>::value ||
  ...
  InlinedVector& operator=(const InlinedVector& other) {
    if (XBSL_PREDICT_FALSE(this == &other)) return *this;
  ...
  reference emplace_back(Args&&... args) {
    size_type s = size();
    assert(s <= capacity());
    if (XBSL_PREDICT_FALSE(s == capacity())) {
  ...
  void shrink_to_fit() {
    const auto s = size();
    if (XBSL_PREDICT_FALSE(!allocated() || s == capacity())) return;
  ...
  friend Hash XbslHashValue(Hash hash, const InlinedVector& inlined_vector) {
 ...
 private:
  ...
  // TODO(xbsl-team): investigate whether the reinterpret_cast is appropriate.
  ...
  union Rep {
    using ValueTypeBuffer =
        xbsl::aligned_storage_t<sizeof(value_type), alignof(value_type)>;
    using AllocationBuffer =
        xbsl::aligned_storage_t<sizeof(Allocation), alignof(Allocation)>;
...
auto InlinedVector<T, N, A>::emplace(const_iterator position, Args&&... args)
  ...
  assert(position >= begin());
  assert(position <= end());
  if (XBSL_PREDICT_FALSE(position == end())) {
...
void InlinedVector<T, N, A>::swap(InlinedVector& other) {
  ...
  if (XBSL_PREDICT_FALSE(this == &other)) return;
...
auto InlinedVector<T, N, A>::InsertWithCount(const_iterator position,
  ...
  if (XBSL_PREDICT_FALSE(n == 0)) return const_cast<iterator>(position);
...
auto InlinedVector<T, N, A>::InsertWithRange(const_iterator position,
    ...
    -> iterator {
  assert(position >= begin() && position <= end());
  if (XBSL_PREDICT_FALSE(first == last)) return const_cast<iterator>(position);
...
#endif  // S2_THIRD_PARTY_XBSL_CONTAINER_INLINED_VECTOR_H_
