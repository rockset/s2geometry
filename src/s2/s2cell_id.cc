...
S2CellId S2CellId::FromFaceIJ(int face, int i, int j) {
  ...
  // of the function.
  uint64 n = xbsl::implicit_cast<uint64>(face) << (kPosBits - 1);
...
string S2CellId::ToString() const {
  if (!is_valid()) {
    return StrCat("Invalid: ", xbsl::Hex(id(), xbsl::kZeroPad16));
  }
...
S2CellId S2CellId::FromDebugString(xbsl::string_view str) {
